...
describe('PolicyEngine', () => {
  it('should return no findings for allowed tools', async () => {
    ...
    expect(await engine.evaluate(msg)).toHaveLength(0);
  });

  it('should return finding for denied tools', async () => {
    const engine = new PolicyEngine({
      defaultAction: 'allow',
      rules: [{ tool: 'exec_cmd', action: 'deny' }],
    });
  ...
  it('should ignore non-tools/call messages', async () => {
    const engine = new PolicyEngine({
      defaultAction: 'deny',
      rules: [],
    });
    ...
    expect(await engine.evaluate(listMsg)).toHaveLength(0);

    const notification: JsonRpcMessage = {
      jsonrpc: '2.0',
      method: 'notifications/initialized',
    };
    expect(await engine.evaluate(notification)).toHaveLength(0);
    ...
    expect(await engine.evaluate(response)).toHaveLength(0);
  });

  it('should include tool name and matched rule in metadata', async () => {
    const engine = new PolicyEngine({
      defaultAction: 'allow',
      rules: [{ tool: 'danger_*', action: 'deny' }],
    });

    const findings = await engine.evaluate(toolsCall('danger_cmd'));
    expect(findings[0]!.metadata?.['toolName']).toBe('danger_cmd');
    expect(findings[0]!.metadata?.['matchedRule']).toBe('danger_*');
  });

  it('should check argument constraints via engine', async () => {
    const engine = new PolicyEngine({
      defaultAction: 'allow',
      rules: [
        {
    ...
    expect(await engine.evaluate(toolsCall('read_file', { path: '/etc/shadow' }))).toHaveLength(1);
    expect(await engine.evaluate(toolsCall('read_file', { path: './readme.md' }))).toHaveLength(0);
  });
});
...
describe('Policy edge cases', () => {
  it('should handle tools/call with missing params.name', async () => {
    ...
    expect(await engine.evaluate(msg)).toHaveLength(0); // Can't extract tool name
  });

  it('should handle tools/call with numeric name', async () => {
    const engine = new PolicyEngine({ defaultAction: 'deny', rules: [] });
    const msg: JsonRpcMessage = {
      jsonrpc: '2.0',
      id: 1,
    ...
    expect(await engine.evaluate(msg)).toHaveLength(0); // Non-string name ignored
  });

  it('should handle empty rules with default-deny', async () => {
    const engine = new PolicyEngine({ defaultAction: 'deny', rules: [] });
...
describe('PolicyEngine — proxy integration logic', () => {
  it('tools/call with params as array (not object)', async () => {
    const engine = new PolicyEngine({ defaultAction: 'deny', rules: [] });
    const msg: JsonRpcMessage = {
      jsonrpc: '2.0',
      id: 1,
  ...
  it('tools/call where params.arguments is a string (not object)', async () => {
    const engine = new PolicyEngine({
      defaultAction: 'allow',
      rules: [
        {
    ...
    const findings = await engine.evaluate(msg);
    // String arguments won't have keys, so constraint check is skipped
    expect(findings).toHaveLength(0);
  });

  it('tools/call where params.arguments is missing entirely', async () => {
    const engine = new PolicyEngine({
      defaultAction: 'allow',
      rules: [
        {
  ...
  it('message with method "tools/call" but no params at all', async () => {
    const engine = new PolicyEngine({ defaultAction: 'deny', rules: [] });
    const msg: JsonRpcMessage = {
      jsonrpc: '2.0',
      id: 1,
  ...
  it('findings have correct severity, category, confidence values', async () => {
    const engine = new PolicyEngine({
      defaultAction: 'allow',
      rules: [{ tool: 'bad_tool', action: 'deny' }],
    });
    const findings = await engine.evaluate(toolsCall('bad_tool'));
    expect(findings).toHaveLength(1);
  ...
  it('metadata includes toolName and matchedRule', async () => {
    const engine = new PolicyEngine({
      defaultAction: 'allow',
      rules: [{ tool: 'write_*', action: 'deny' }],
    });
    const findings = await engine.evaluate(toolsCall('write_config'));
    expect(findings).toHaveLength(1);
  ...
  it('metadata matchedRule is undefined when denied by default action', async () => {
    const engine = new PolicyEngine({ defaultAction: 'deny', rules: [] });
    const findings = await engine.evaluate(toolsCall('anything'));
    expect(findings).toHaveLength(1);
