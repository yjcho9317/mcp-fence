...
describe('W5-SEC: Monitor vs Enforce Mode', () => {
  it('PolicyEngine returns findings for denied tools regardless of mode', async () => {
    ...
    const findings = await engine.evaluate(message);
  ...
  it('PolicyEngine returns empty findings for allowed tools', async () => {
    const policyConfig: PolicyConfig = {
      defaultAction: 'allow',
  ...
  it('PolicyEngine ignores non-tools/call messages', async () => {
    const policyConfig: PolicyConfig = {
      defaultAction: 'deny',
      rules: [],
    };
  ...
  it('PolicyEngine ignores response messages', async () => {
    const policyConfig: PolicyConfig = {
      defaultAction: 'deny',
      rules: [],
    };
    ...
    const findings = await engine.evaluate(response);
    expect(findings).toHaveLength(0);
  });

  it('VULN-INFO: attacker can probe denied tools via error messages', async () => {
    ...
    const allowed = await engine.evaluate(toolsCallMessage('safe_tool'));
    const denied = await engine.evaluate(toolsCallMessage('dangerous_tool'));
...
describe('W5-SEC: Config Validation Security', () => {
  ...
  describe('Rule tool pattern validation', () => {
    ...
    it('VULN: empty tool name in request matches empty rule', async () => {
...
describe('W5-SEC: PolicyEngine Integration', () => {
  it('tools/call without params is ignored', async () => {
    const engine = new PolicyEngine({
      defaultAction: 'deny',
      rules: [],
    });
  ...
  it('tools/call with non-string name is ignored', async () => {
    const engine = new PolicyEngine({
      defaultAction: 'deny',
      rules: [],
    });
  ...
  it('VULN: tools/call with numeric name bypasses all policy', async () => {
  ...
  it('VULN: tools/call with null name bypasses all policy', async () => {
    const engine = new PolicyEngine({
      defaultAction: 'deny',
      rules: [{ tool: '*', action: 'deny' }],
    });
  ...
  it('VULN: method with different casing is not checked', async () => {
    const engine = new PolicyEngine({
      defaultAction: 'deny',
      rules: [{ tool: '*', action: 'deny' }],
    });
