...
program
  ...
  .action(async (opts: { config?: string; mode?: string; logLevel?: string }, cmd: Command) => {
    ...
    log.info(`mcp-fence v0.1.0 — mode: ${config.mode}`);
    ...
    // Audit logging — SQLite database in CWD
    const dbPath = resolve(process.cwd(), 'mcp-fence-audit.db');
...
program
  ...
  .action((opts: { config?: string }) => {
    ...
    process.stdout.write('mcp-fence v0.1.0\n\n');
...
program
  ...
  .option('--db <path>', 'Path to audit database', 'mcp-fence-audit.db')
  ...
  .action((opts: {
    ...
    // Validate db path is within CWD to prevent path traversal
    const cwd = resolve(process.cwd());
    if (!dbPath.startsWith(cwd)) {
      log.error(`Database path must be within current directory: ${opts.db}`);
      process.exit(1);
    }
